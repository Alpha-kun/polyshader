      ...
      for (int y = max(miny, 0); y<min(maxy, height); y++) {
        for (int x = max(minx, 0); x<min(maxx, width); x++) {
          ...
          if (dD<depth[y*width+x]) {
            ...
            PVector N = PVector.mult(vertexN[i], bary.x).add(PVector.mult(vertexN[j], bary.y)).add(PVector.mult(vertexN[k], bary.z));
            N.normalize();
            ...
            for (PointLight pl : lights) {
              ...
              PVector R = PVector.sub(PVector.mult(N, 2*N.dot(L)), L);
              PVector Id = hadamard(material.Kd, pl.Ip).mult(max(L.dot(N), 0));
              PVector Is = hadamard(material.Ks, pl.Ip).mult(pow(max(R.dot(V), 0), material.n));
              Itot.add(PVector.add(Id, Is));
            }
            
            color c = color(min(Itot.x, 1.0)*255, min(Itot.y, 1.0)*255, min(Itot.z, 1.0)*255);
            pixels[y*width+x] = c;
          }
        }
      }
